...
case "$COMMAND" in
  ...
  semver-check)
    ...
    OLD_SO="$OLD_DIR/build/liba_curl_gcloud_plugin_shared.so"
    NEW_SO="build/liba_curl_gcloud_plugin_shared.so"
  ...
  coverage)
    ...
    llvm-cov report "$COV_BUILD_DIR/liba_curl_gcloud_plugin_debug.a" \
