...
typedef struct {
    curl_sink_interface_t iface; aml_buffer_t *resp;
    void *cb; void *cb_arg;
} base_sink_t;

static bool s_init(curl_sink_interface_t *iface, long cl) {
    base_sink_t *s = (void *)iface;
    if (s->resp) aml_buffer_clear(s->resp);
    else s->resp = aml_buffer_init(cl > 0 ? cl + 1 : 4096);
    return s->resp != NULL;
}
static size_t s_write(const void *ptr, size_t sz, size_t nm, curl_sink_interface_t *iface) {
    base_sink_t *s = (void *)iface; aml_buffer_append(s->resp, ptr, sz * nm); return sz * nm;
}
static void s_destroy(curl_sink_interface_t *iface) {
    base_sink_t *s = (void *)iface; if (s->resp) aml_buffer_destroy(s->resp);
}

static char *unescape_gmail_string(aml_pool_t *pool, const char *raw) {
    if (!raw) return NULL;
    size_t len = strlen(raw); char *out = aml_pool_alloc(pool, len + 1); char *p = out;
...
static gcloud_v1_gmail_contact_t *parse_contacts(aml_pool_t *pool, const char *clean_val, size_t *out_count) {
    ...
    char *copy = aml_pool_strdup(pool, clean_val); size_t max_contacts = 1;
    ...
    size_t count = 0; char *p = copy;
    while (*p) {
        while (*p == ' ' || *p == '\t' || *p == '\n' || *p == '\r') p++;
        if (!*p) break;
        char *start = p; bool in_quotes = false; char *email_start = NULL; char *email_end = NULL;
        while (*p) {
        ...
        bool has_comma = (*p == ','); *p = '\0';
        if (email_start && email_end && email_end > email_start) {
            *email_end = '\0'; contacts[count].email = email_start;
            char *name_end = email_start - 2;
            while (name_end >= start && (*name_end == ' ' || *name_end == '\t' || *name_end == '"' || *name_end == '<')) { *name_end = '\0'; name_end--; }
            while (*start == ' ' || *start == '\t' || *start == '"') start++;
            if (strlen(start) > 0) contacts[count].name = start;
        } else {
            while (*start == ' ' || *start == '\t' || *start == '"') start++;
            char *end = start + strlen(start) - 1;
            while (end > start && (*end == ' ' || *end == '\t' || *end == '"')) { *end = '\0'; end--; }
            if (strlen(start) > 0) contacts[count].email = start;
        }
        count++; if (has_comma) p++;
    }
    *out_count = count; return contacts;
}

static void walk_dom_for_text(lxb_dom_node_t *node, aml_buffer_t *out) {
    if (node->type == LXB_DOM_NODE_TYPE_TEXT) {
        size_t len = 0; const lxb_char_t *text = lxb_dom_node_text_content(node, &len);
        ...
        if (node->local_name == LXB_TAG_P || node->local_name == LXB_TAG_DIV || node->local_name == LXB_TAG_BR) aml_buffer_append(out, "\n", 1);
    }
    lxb_dom_node_t *child = lxb_dom_node_first_child(node);
    while (child != NULL) { walk_dom_for_text(child, out); child = lxb_dom_node_next(child); }
}

static uint8_t *html_to_plain_text(aml_pool_t *pool, const uint8_t *html_data, size_t html_len, size_t *out_len) {
    lxb_html_document_t *document = lxb_html_document_create();
    if (!document) return NULL;
    if (lxb_html_document_parse(document, (const lxb_char_t *)html_data, html_len) != LXB_STATUS_OK) {
        lxb_html_document_destroy(document); return NULL;
    }
    aml_buffer_t *out = aml_buffer_init(html_len / 2 + 1024);
    if (!out) { lxb_html_document_destroy(document); return NULL; }
    ...
    *out_len = aml_buffer_length(out) - 1;
    ...
    aml_buffer_destroy(out); lxb_html_document_destroy(document);
    return result;
}
...
static uint8_t *b64dec_websafe(aml_pool_t *pool, const char *b64, size_t *out_len) {
    ...
    for (size_t i = 0; i < len; i++) {
        ...
        if (v == -1) continue;
        val = (val << 6) | v; valb += 6;
        if (valb >= 0) { buf[j++] = (val >> valb) & 0xFF; valb -= 8; }
    }
    *out_len = j; return buf;
}
...
typedef struct { base_sink_t b; gcloud_v1_gmail_messages_list_cb cb; } sink_list_t;
...
static void list_complete(curl_sink_interface_t *iface, curl_event_request_t *req) {
    sink_list_t *s = (void *)iface; aml_pool_t *pool = iface->pool;
    ajson_t *json = ajson_parse_string(pool, aml_buffer_data(s->b.resp));
    if (!json || ajson_is_error(json)) { if (s->cb) s->cb(s->b.cb_arg, req, false, NULL, 0, NULL, 0); return; }
...
curl_sink_interface_t *gcloud_v1_gmail_messages_list_sink(curl_event_request_t *req, gcloud_v1_gmail_messages_list_cb cb, void *cb_arg) {
    if (!req) return NULL;
    sink_list_t *s = aml_pool_zalloc(req->pool, sizeof *s);
    s->cb = cb; s->b.cb_arg = cb_arg; s->b.iface.pool = req->pool;
    s->b.iface.init = s_init; s->b.iface.write = s_write;
    s->b.iface.failure = list_failure; s->b.iface.complete = list_complete; s->b.iface.destroy = s_destroy;
    curl_event_request_sink(req, (curl_sink_interface_t *)s, NULL);
    return (curl_sink_interface_t *)s;
}

typedef struct { base_sink_t b; gcloud_v1_gmail_message_get_cb cb; bool extract_html_text; } sink_get_t;
...
static void get_complete(curl_sink_interface_t *iface, curl_event_request_t *req) {
    sink_get_t *s = (void *)iface; aml_pool_t *pool = iface->pool;
    ajson_t *json = ajson_parse_string(pool, aml_buffer_data(s->b.resp));
    ...
    msg->snippet = unescape_gmail_string(pool, ajsono_scan_str(json, "snippet", NULL));
    msg->internal_date = strtoull(ajsono_scan_str(json, "internalDate", "0"), NULL, 10);
    ...
    if (payload) {
        ...
        if (headers && ajson_is_array(headers)) {
            ...
            for (ajsona_t *el = ajsona_first(headers); el; el = ajsona_next(el)) {
                ...
                char *clean_val = unescape_gmail_string(pool, ajsono_scan_str(el->value, "value", ""));
                msg->header_values[i] = clean_val;
                if (strcasecmp(msg->header_names[i], "Subject") == 0) msg->subject = clean_val;
                else if (strcasecmp(msg->header_names[i], "From") == 0) msg->from = parse_contacts(pool, clean_val, &msg->num_to); // Dummy out_count var to satisfy signature, From is usually 1
                else if (strcasecmp(msg->header_names[i], "To") == 0) msg->to = parse_contacts(pool, clean_val, &msg->num_to);
                else if (strcasecmp(msg->header_names[i], "Cc") == 0) msg->cc = parse_contacts(pool, clean_val, &msg->num_cc);
                else if (strcasecmp(msg->header_names[i], "Bcc") == 0) msg->bcc = parse_contacts(pool, clean_val, &msg->num_bcc);
                else if (strcasecmp(msg->header_names[i], "Delivered-To") == 0) msg->delivered_to = clean_val;
                else if (strcasecmp(msg->header_names[i], "List-Id") == 0 || strcasecmp(msg->header_names[i], "X-Google-Group-Id") == 0 || strcasecmp(msg->header_names[i], "Mailing-List") == 0) {
                    msg->list_id = clean_val; msg->is_group = true;
                }
                i++;
            }
        }
        ...
        if (b64) is_html = true; else b64 = find_mime_part(payload, "text/plain");
        if (!b64) {
            ajson_t *body = ajsono_scan(payload, "body");
            b64 = body ? ajsono_scan_str(body, "data", NULL) : NULL;
        }
        if (b64) {
            size_t raw_len = 0; uint8_t *raw_data = b64dec_websafe(pool, b64, &raw_len);
            if (is_html && s->extract_html_text) {
                size_t text_len = 0; uint8_t *text_data = html_to_plain_text(pool, raw_data, raw_len, &text_len);
                if (text_data) { msg->body_data = text_data; msg->body_len = text_len; }
                else { msg->body_data = raw_data; msg->body_len = raw_len; }
            } else {
                msg->body_data = raw_data; msg->body_len = raw_len;
            }
        }
        ...
        if (msg->num_attachments > 0) {
            ...
            size_t idx = 0; extract_attachments(pool, payload, msg->attachments, &idx);
        }
    } else {
        const char *raw_b64 = ajsono_scan_str(json, "raw", NULL);
        if (raw_b64) msg->body_data = b64dec_websafe(pool, raw_b64, &msg->body_len);
    }
    if (s->cb) s->cb(s->b.cb_arg, req, true, msg);
}
curl_sink_interface_t *gcloud_v1_gmail_message_get_sink(curl_event_request_t *req, bool extract_html_text, gcloud_v1_gmail_message_get_cb cb, void *cb_arg) {
    if (!req) return NULL;
    sink_get_t *s = aml_pool_zalloc(req->pool, sizeof *s);
    s->cb = cb; s->b.cb_arg = cb_arg; s->extract_html_text = extract_html_text;
    s->b.iface.pool = req->pool; s->b.iface.init = s_init; s->b.iface.write = s_write;
    s->b.iface.failure = get_failure; s->b.iface.complete = get_complete; s->b.iface.destroy = s_destroy;
    curl_event_request_sink(req, (curl_sink_interface_t *)s, NULL);
    return (curl_sink_interface_t *)s;
}

typedef struct { base_sink_t b; gcloud_v1_gmail_attachment_get_cb cb; } sink_att_t;
...
static void att_complete(curl_sink_interface_t *iface, curl_event_request_t *req) {
    sink_att_t *s = (void *)iface; aml_pool_t *pool = iface->pool;
    ajson_t *json = ajson_parse_string(pool, aml_buffer_data(s->b.resp));
    ...
    if (!b64_data) { if (s->cb) s->cb(s->b.cb_arg, req, false, NULL, 0); return; }
    size_t file_len = 0; uint8_t *file_data = b64dec_websafe(pool, b64_data, &file_len);
    if (s->cb) s->cb(s->b.cb_arg, req, true, file_data, file_len);
}
curl_sink_interface_t *gcloud_v1_gmail_attachment_get_sink(curl_event_request_t *req, gcloud_v1_gmail_attachment_get_cb cb, void *cb_arg) {
    if (!req) return NULL;
    sink_att_t *s = aml_pool_zalloc(req->pool, sizeof *s);
    s->cb = cb; s->b.cb_arg = cb_arg; s->b.iface.pool = req->pool; s->b.iface.init = s_init;
    s->b.iface.write = s_write; s->b.iface.failure = att_failure; s->b.iface.complete = att_complete; s->b.iface.destroy = s_destroy;
    curl_event_request_sink(req, (curl_sink_interface_t *)s, NULL);
    return (curl_sink_interface_t *)s;
}

typedef struct { base_sink_t b; gcloud_v1_gmail_message_id_cb cb; } sink_msg_id_t;
...
static void msg_id_complete(curl_sink_interface_t *iface, curl_event_request_t *req) {
    sink_msg_id_t *s = (void *)iface; aml_pool_t *pool = iface->pool;
    aml_buffer_append(s->b.resp, "", 1);
    ajson_t *json = ajson_parse_string(pool, aml_buffer_data(s->b.resp));
    if (!json || ajson_is_error(json)) { if (s->cb) s->cb(s->b.cb_arg, req, true, NULL, NULL); return; }
...
curl_sink_interface_t *gcloud_v1_gmail_message_id_sink(curl_event_request_t *req, gcloud_v1_gmail_message_id_cb cb, void *cb_arg) {
    if (!req) return NULL;
    sink_msg_id_t *s = aml_pool_zalloc(req->pool, sizeof *s);
    s->cb = cb; s->b.cb_arg = cb_arg; s->b.iface.pool = req->pool; s->b.iface.init = s_init;
    s->b.iface.write = s_write; s->b.iface.failure = msg_id_failure; s->b.iface.complete = msg_id_complete; s->b.iface.destroy = s_destroy;
    curl_event_request_sink(req, (curl_sink_interface_t *)s, NULL);
    return (curl_sink_interface_t *)s;
}