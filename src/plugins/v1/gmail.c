...
static bool _on_prepare(curl_event_request_t *req) {
    ...
    /* 2. Rebuild the URL dynamically.
       This ensures that if the rate_manager delays the request, we don't
       accidentally append duplicate query parameters. */
...
/* Updated _start to accept the API cost (weight) */
       ...
       const char *base_endpoint, const char *method, const char *path, double weight)
{
    ...
    /* Base URL is set initially, but _on_prepare will rebuild it with query params */
    ...
    /* Enforce Gmail Quotas automatically with the correct endpoint weight */
    ...
    const char *user_id)
{
    ...
    /* messages.list costs 1 quota unit */
    ...
    const char *attachment_id)
{
    ...
    /* attachments.get costs 5 quota units */
    return _start(loop, token_id, base_endpoint, "GET", path, 5.0);
}
